...
static const int8 dmp_memory[DMP_CODE_SIZE] = {
    /* bank # 0 */
...
#define DPM_LOAD_CHUNK_SIZE  (16)
static void dmp_load_firmware_chunk( int16 addr, int8 chunk_size )
{
	int8 write_chunk[DPM_LOAD_CHUNK_SIZE];
	int8 read_chunk[DPM_LOAD_CHUNK_SIZE];
	
	for( int8 i=0; i<chunk_size; ++i ) {
		write_chunk[i] = dmp_memory[addr+i];
	}
	mpu_write_mem(addr, chunk_size, write_chunk);
	mpu_read_mem(addr, chunk_size, read_chunk);
#if defined(DMP_FIRMWARE_CHECK)
	if( memcmp( read_chunk, write_chunk, chunk_size ) != 0 ) {
		mpu_error_handler( MPU_ERROR_WRITE_VERIFY_FAILED );
	}
#endif
}
...
void dmp_load_motion_driver_firmware()
{
	...
	for( addr = 0; addr < (DMP_CODE_SIZE-DPM_LOAD_CHUNK_SIZE);
			addr += DPM_LOAD_CHUNK_SIZE )
		dmp_load_firmware_chunk( addr, DPM_LOAD_CHUNK_SIZE );
	
	dmp_load_firmware_chunk( addr, DMP_CODE_SIZE - addr );
	
	mpu_cnf_dmp_firmware( DMP_START_ADDRESS, DMP_SAMPLE_RATE );
}
...
{
    int8 regs_end[12] = {DINAFE, DINAF2, DINAAB,
    ...
    int16 div = (DMP_SAMPLE_RATE / rate) - 1;
    tmp[0] = (int8)((div >> 8) & 0xFF);
    tmp[1] = (int8)(div & 0xFF);
    mpu_write_mem(D_0_22, 2, tmp);
	mpu_write_mem(CFG_6, 12, regs_end);
}
 ...
 *  @return     0 if successful.
 */
int1 dmp_read_fifo_quat( signed int32 *quat, unsigned char *more )
{
#if 1
    /* Get a packet. */
	unsigned int8 fifo_data[16];
    if ( !mpu_read_fifo_stream( 16, fifo_data, more ) )
        return 0;
#else
	unsigned int8 fifo_data[16] = 
			{0xff,0xfd,0x1b,0x47,
...
void dmp_enable_feature( int16 mask )
{
    ...
    tmp[0] = (int8)((GYRO_SF >> 24) & 0xFF);
    tmp[1] = (int8)((GYRO_SF >> 16) & 0xFF);
    tmp[2] = (int8)((GYRO_SF >> 8) & 0xFF);
    tmp[3] = (int8)(GYRO_SF & 0xFF);
...
{
	int8 regs_continuous[11] =
        {0xd8, 0xb1, 0xb9, 0xf3, 0x8b, 0xa3, 0x91, 0xb6, 0x09, 0xb4, 0xd9};
    int8 regs_gesture[11] =
        {0xda, 0xb1, 0xb9, 0xf3, 0x8b, 0xa3, 0x91, 0xb6, 0xda, 0xb4, 0xda};

    switch (mode) {
		case DMP_INT_CONTINUOUS:
			mpu_write_mem( CFG_FIFO_ON_EVENT, 11, regs_continuous );
